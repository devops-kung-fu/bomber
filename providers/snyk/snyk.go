...
func (Provider) Scan(purls []string, credentials *models.Credentials) (packages []models.Package, err error) {
	...
	orgID, err := getOrgID(credentials.ProviderToken, credentials.ProviderURL)
	if err != nil {
	...
	for _, pp := range purls {
		...
		go func(purl string) {
			...
			vulns, err := getVulnsForPurl(purl, orgID, credentials.ProviderToken, credentials.ProviderURL)
			if err != nil {
